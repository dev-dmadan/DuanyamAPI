...
async function getChartData({MainFilter, CustomFilter}) {
    try {
        ...
        const req = await fetch(`${APP_URL}/dashboard/api/realisasi-produksi-per-lokasi?SecretKey=${secretKey}`, {
            method: 'POST',
            headers: headers,
            body: JSON.stringify({
...
async function getDetailData({MainFilter, CustomFilter, Page = 1, isExport = false}) {
    try {
        ...
        const headers = new Headers();
        headers.append("Content-Type", "application/json");
        const req = await fetch(`${APP_URL}/dashboard/api/detail-realisasi-produksi-per-lokasi?SecretKey=${secretKey}`, {
            method: 'POST',
            headers: headers,
            body: JSON.stringify({
                MainFilter: MainFilter ?? null,
